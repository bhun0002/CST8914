...
// Add mousedown event listener on window to handle clicks outside the menu
  ...
  setFocusToMenuitem(newMenuitem) {
    // Loop through each menu item and set tabIndex based on focus
    this.menuitemNodes.forEach(function (item) {
      if (item === newMenuitem) {
        item.tabIndex = 0; // Set the tabIndex of the focused element to "0"
        item.focus(); // Apply focus to the new element
      } else {
        item.tabIndex = -1; // Set tabIndex of the previously focused element to "-1"
      }
    });
  }
